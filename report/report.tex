...
We want to draw the reader's attention to the fact that the actual values \stt{x} and \stt{y} did not play any role in this process. Moreover, while the exact content of the \stt{Map} underlying the \stt{map} variable was unknown, we knew enough about it from the precondition of \stt{test} to successfully reduce the program to a simple expression. We formalise this whole process in the next section.
...
\begin{itemize}
\item \textit{Partial evaluation} is the process of executing a program in the presence of partial inputs.
\item \textit{Symbolic evaluation} is the process of gathering knowledge about symbolic values in a program by executing it.
\item A \textit{path condition} is a quantifier-free formula in conjunctive normal form which encodes the knowledge we have gathered so far about some symbolic values.
\end{itemize}

Combining both techniques into a single evaluation procedure, ie. a procedure which partially evaluates a program with symbolic inputs while maintaining a \textit{path condition}, has been shown to increase both performance and reasoning abilities of verifications systems in some cases \cite{partialsym}. We call such a procedure a \textit{symbolic partial evaluator}.\\



When encountering a boolean expression, the evaluator can then make use of the path condition to determine the value of that expression.  both partial and symbolic evaluation at the same time 

For example, when the evaluator encounters an expression $e$ of type \stt{Boolean} under a path condition $p$, 
