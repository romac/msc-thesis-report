...
Unfortunately, Stainless is unable to prove the verification condition corresponding to \stt{test}'s postcondition.

If one were to manually unfold \stt{foldLeft}'s definition in the code above, one would end up with the following expression for the \stt{res} variable:
...
The evaluator keeps track of the current \textit{path condition}, denoted by $\Delta$, as well as its associated bindings.
...
holding the behavior associated with each actor reference, the list of in-flight messages between any two actors, as well as a trace of the execution up to that point, modelled as a list of \InlineS{Transition}s.
