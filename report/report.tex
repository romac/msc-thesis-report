...
We argue that, when ran over a program that Stainless deems terminating, the symbolic evaluator whose semantics are given in \RefFig{fig:symbolicsem} terminates as well. We will first note that, for a recursive function expressed in PureScala to be  terminating, the recursive calls must be guarded by a conditional expression, as it would otherwise call itself repeatedly without end. Because we only ever unfold a recursive function  (Wrong, find counter example)
...
\subsubsection*{Path Condition Satisfiability Improvements}
