...
(n >= 0 && n != 0) ==> (n - 1 >= 0)
\end{ShortCode}
%$\forall n.\, n \geq 0 \implies factorial(n) \geq 0$.
...
This procedure is formalized in rules 10, 11, and 12 of \RefFig{fig:symbolicsem}.

\subsubsection*{Evaluation of pattern matches}

Pattern matches are converted to an equivalent conditional expression with explicit type checks, casts, and field accesses, before being fed back to the evaluator. The corresponding rule (21) can be found in \RefFig{fig:symbolicsem}.
...
%This property is formalized in \RefFig{lst:recsafe}.
...
The corresponding rules (\ref{fig:symbolicsem:fi1}, \ref{fig:symbolicsem:fi2}, and \ref{fig:symbolicsem:fi3}) can be found in \RefFig{fig:symbolicsem}.
...
%  \empty
%}{
%  \langle \stt{$c$.spawn(b, id)}, c \rangle \longrightarrow (b',\ \stt{$c$.copy(toSend $\mapsto$ ($n$, $m$) $::$ $c$.toSend)})
%}
%\tag{I-SPAWN}
%\end{equation}
%
...
%
%\RefCode{lst:LockService} shows the implementation of a lock service using our framework. In this case study, an actor acts as a server holding a lock on some resource, while a number of other actors (the "agents") act as clients of the lock service, each potentially trying to acquire the lock on the resource. To model a variable number of actors with the same implementation, we define their reference as a case class parametrized by a an identifier.
%
...
\begin{Code}{PureScala}{lst:spawncode}{Actor system with a dynamic topology}
...
Because linear logic does not allow contraction, we must also ensure that no linear term is \textit{dropped}, that is to say, that it is \textit{consumed} at least once. To this end, we first collect all linear variables being introduced in a function definition, for example as a parameter to the function, in a variable binding, or within a pattern (even as a wildcard). Then, after having ran the type-checking algorithm described in the previous section, we can make use of the resulting \textit{usage context} $\Delta$ to check whether each and every of those variable has indeed been \textit{consumed} at some point, and reject the program otherwise.
...
\begin{landscape}
\section{Appendix}
\subsection{Implementation of an interpreter for a simple programming language}
\lstinputlisting[language=PureScala, style=LongCode]{code/comp.scala}

\subsection{Implementation of an algebra over a key-value store}
\lstinputlisting[language=PureScala, style=LongCode]{code/symeval.scala}

\subsection{Implementation of a linear list}
\lstinputlisting[language=PureScala, style=LongCode]{code/linlist.scala}

\subsection{Implementation of an actor system in PureScala}
\lstinputlisting[language=PureScala, style=LongCode]{code/actors/package.scala}
\lstinputlisting[language=PureScala, style=LongCode]{code/actors/ActorSystem.scala}

\subsection{Examples of actor systems}
\lstinputlisting[language=PureScala, style=LongCode]{code/actors/Counting.scala}
\lstinputlisting[language=PureScala, style=LongCode]{code/actors/ReplicatedCounter.scala}
\lstinputlisting[language=PureScala, style=LongCode]{code/actors/Spawn.scala}

\subsection{Shim for running an actor system on top of Akka}
\lstinputlisting[language=PureScala, style=LongCode]{code/actors/shim.scala}
